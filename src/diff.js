...
const makeDiff = (tree1, tree2) => {
  ...
  const result = keys.map((key) => {
    if (!_.has(tree2, key)) {
      return { status: 'removed', key, val1: tree1[key] };
    }
    if (!_.has(tree1, key)) {
      return { status: 'added', key, val1: tree2[key] };
    }
    if (_.isEqual(tree1[key], tree2[key])) {
      return { status: 'unchanged', key, val1: tree1[key] };
    }
    if (_.isObject(tree1[key]) && _.isObject(tree2[key])) {
      return { status: 'ancestor', key, children: makeDiff(tree1[key], tree2[key]) };
    }
    return {
      status: 'changed', key, val1: tree1[key], val2: tree2[key],
    };
  });
  return result;
};
