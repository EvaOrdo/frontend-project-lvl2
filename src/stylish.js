...
const stringifyObjValue = (value, depth) => {
  if (!_.isObject(value)) {
    return value;
  }
  const space = '  ';
...
const formatTree = (ast) => {
  const iter = (tree, depth) => {
    const result = tree.map((node) => {
      ...
      switch (status) {
        case 'unchanged':
          return `${space.repeat(depth)}  ${key}: ${stringifyObjValue(val1, depth)}`;
        case 'added':
          return `${space.repeat(depth)}+ ${key}: ${stringifyObjValue(val1, depth)}`;
        case 'remove':
          return `${space.repeat(depth)}- ${key}: ${stringifyObjValue(val1, depth)}`;
        case 'changed':
          return `${space.repeat(depth)}- ${key}: ${stringifyObjValue(val1, depth)}\n${space.repeat(depth)}+ ${key}: ${stringifyObjValue(val2, depth)}`;
        case 'ancestor':
